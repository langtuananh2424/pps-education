...
## 13. Website công khai ppsvietnam.edu.vn (chuyển từ VPS cũ, 2026-09)

Website giới thiệu trung tâm (Next.js + NestJS + Postgres 15, source
`../pps-center/`, repo GitHub riêng `ppsvn-web`) chạy CÙNG server này dưới compose project `ppsvn`, tách
biệt hoàn toàn với 2 stack trên: port host `127.0.0.1:3100` (frontend) /
`3101` (backend) / `5434` (postgres), subnet `172.30.0.0/24`, thư mục
`/opt/pps-center/{src,data/db}`. Nginx `ppsvietnam.edu.vn.conf` + 2 hostname
`ppsvietnam.edu.vn`/`www.ppsvietnam.edu.vn` thêm vào ingress tunnel
`pps-education`. Backup đã gộp vào `backup-db.sh` (stack `ppsvn`).

Quy trình chuyển đầy đủ (dump DB từ VPS, restore, nginx, Cloudflare, rollback):
xem repo [ppsvn-web](https://github.com/langtuananh2424/ppsvn-web) `deploy/MIGRATION.md`.
