...
sub authcell {
    ...
    # /api/auth/me và /api/auth/me/password yêu cầu JWT (rule riêng trong
    # SecurityConfig, khai báo TRƯỚC "/api/auth/**" permitAll) -- không thuộc
    # nhóm Công khai như các endpoint /api/auth/* còn lại (login/refresh/logout).
    return 'JWT' if $path eq '/api/auth/me' || $path eq '/api/auth/me/password';
