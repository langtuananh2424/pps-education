        ...
        return (
          <div key={col.id} className="bg-slate-100 rounded-xl p-4 flex flex-col min-h-[500px] kanban-column">
            <div className="flex items-center justify-between mb-3.5 pb-2 border-b border-slate-200">
              ...
              <span className="text-sm font-mono font-bold bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full">{colTasks.length}</span>
            </div>
            ...
            <div className="space-y-3 flex-1 overflow-y-auto max-h-[520px] pr-0.5">
              ...
              {colTasks.length === 0 && (
                <div className="h-28 border-2 border-dashed border-slate-200 rounded-lg flex items-center justify-center text-sm text-slate-400 italic">
                  Trống
                </div>
              )}
            </div>
