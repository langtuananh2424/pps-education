    ...
    <Modal open onClose={onClose} title={task.title} description={`Mã tác vụ: ${task.id}`} size="lg">
      <div className="space-y-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-700 leading-relaxed bg-slate-50 p-3 rounded-lg border border-slate-100">{task.description}</p>
        </div>

        <div className="grid grid-cols-2 gap-4 bg-slate-50 p-3.5 rounded-lg border border-slate-100">
          <div>
            <span className="text-sm uppercase font-bold tracking-wider text-slate-400 block font-display">Cập nhật Trạng thái</span>
            <Select
              value={task.status}
              onChange={(e) => onChangeStatus(e.target.value as Task["status"])}
              className="mt-1 w-full bg-white border border-slate-200 text-sm px-2.5 py-1.5 rounded-lg focus:outline-none font-semibold text-slate-700"
            >
          ...
          <div className="text-right flex flex-col justify-end">
            <span className="text-sm text-slate-400 font-medium">Bàn giao bởi:</span>
            <span className="text-sm font-bold text-slate-800 block mt-0.5">{task.assignedBy}</span>
          </div>
        </div>
        ...
        <div className="space-y-3.5">
          ...
          <div className="space-y-2.5">
            ...
            {(task.comments || []).length === 0 && <p className="text-sm text-slate-400 italic">Chưa có bình luận hay báo cáo phản hồi nào.</p>}
          </div>
