        ...
        return (
          <div key={status} className="bg-slate-100 rounded-xl p-3.5 flex flex-col min-h-[420px]">
            <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-200">
              ...
              <span className="text-sm font-mono font-bold bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded-full">{colItems.length}</span>
            </div>
            ...
            <div className="space-y-2.5 flex-1 overflow-y-auto max-h-[460px] pr-0.5">
              {colItems.map((a) => (
                ...
                >
                  <span className="text-sm font-bold text-slate-900 group-hover:text-brand-orange transition-colors line-clamp-2">{a.taskTitle}</span>
                  <div className="border-t border-slate-100 pt-2 mt-2.5 flex items-center justify-between">
                    <span className="text-sm font-mono text-slate-400">#{a.id}</span>
                    {a.startedAt && (
              ...
              {colItems.length === 0 && (
                <div className="h-24 border-2 border-dashed border-slate-200 rounded-lg flex items-center justify-center text-sm text-slate-400 italic">
                  Trống
                </div>
              )}
            </div>
