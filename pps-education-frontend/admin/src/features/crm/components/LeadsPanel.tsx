    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-soft overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
          <span className="text-xs font-bold text-slate-700 font-display">Danh sách Khách hàng Tiềm năng</span>
          <span className="text-[10px] font-mono font-bold bg-white px-2 py-0.5 border rounded-full text-slate-500">Landing Page API Active</span>
        </div>
              ...
              <div className="flex items-center gap-3">
                ...
                {lead.status === "QUALIFIED" && (
                  <button
                    ...
                    className="bg-brand-gradient hover:opacity-95 text-white font-bold text-[10px] px-2.5 py-1 rounded flex items-center gap-1 shadow-soft"
        ...
        {selectedLead ? (
          <div className="space-y-4">
            <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
              <div>
                <span className="text-[10px] font-mono font-bold text-slate-400">HỒ SƠ TƯ VẤN: {selectedLead.id}</span>
                <h3 className="text-xs font-bold text-slate-800">{selectedLead.fullName}</h3>
              </div>
              <button onClick={() => setSelectedLeadId(null)} className="text-slate-400 hover:text-slate-700 text-xs">
                Đóng
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3 pt-2">
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold tracking-wider text-slate-500">Nội dung cuộc gọi</label>
                <textarea
                  required
            ...
            {selectedLead.status === "QUALIFIED" && (
              <div className="border-t border-slate-100 pt-3 flex flex-col gap-2">
                <p className="text-[11px] text-slate-400 italic">Học sinh đã hoàn thành tư vấn, click nút dưới để tự động tạo hồ sơ học viên chính thức.</p>
                <button
