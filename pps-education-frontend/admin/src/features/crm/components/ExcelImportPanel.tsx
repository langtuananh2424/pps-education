    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-soft space-y-4">
        ...
        >
          ...
          <div>
            <span className="text-sm font-bold text-slate-700 block">Click để tải lên tệp tin danh sách mẫu (Excel)</span>
            <span className="text-sm text-slate-400 mt-1 block">Hỗ trợ định dạng: .xlsx, .xls, .csv</span>
          </div>
        </div>

        {fileName && (
          <div className="p-3 bg-slate-50 border rounded-lg flex items-center justify-between text-sm animate-in fade-in">
            <div className="flex items-center gap-2">
              <FileSpreadsheet className="w-5 h-5 text-emerald-600 shrink-0" />
              <span className="font-semibold text-slate-800 truncate max-w-sm">{fileName}</span>
            </div>
            <span className="text-sm bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded font-bold font-mono">Phân tích thành công</span>
          </div>
        )}

        {importReport && (
          <div className="space-y-4 border-t border-slate-100 pt-4 animate-in slide-in-from-bottom-2 duration-150">
            <h4 className="text-sm font-bold text-slate-800 font-display">Biên bản đối chiếu & Nhập hồ sơ hàng loạt</h4>

            <div className="grid grid-cols-3 gap-4 text-center">
              <div className="bg-slate-50 p-3 rounded border">
                <span className="text-sm text-slate-400 font-bold block uppercase">Tổng dòng đọc được</span>
                <span className="text-xl font-bold text-slate-800 font-mono">{importReport.total} dòng</span>
              </div>
              <div className="bg-emerald-50 p-3 rounded border border-emerald-100">
                <span className="text-sm text-emerald-600 font-bold block uppercase">Hợp lệ & Gán lớp</span>
              ...
              <div className="bg-rose-50 p-3 rounded border border-rose-100">
                <span className="text-sm text-rose-600 font-bold block uppercase">Cảnh báo trùng lặp</span>
            ...
            <div className="bg-rose-50/50 p-4 rounded-lg border border-rose-100 space-y-2">
              ...
              <p className="text-sm text-slate-400 mt-1">*Hệ thống tự động bỏ qua 2 dòng trên để bảo đảm tính thống nhất và không bị sai lệch cơ sở dữ liệu.</p>
            </div>
          </div>
        )}
      </div>
