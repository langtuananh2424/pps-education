    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card padded className="p-4">
          <span className="text-sm text-slate-400 font-bold block uppercase tracking-wider font-display">Ý kiến đối tác mới</span>
          <div className="text-2xl font-bold text-brand-red font-display mt-1">
            {mockFeedbackTickets.filter((f) => f.status === "NEW").length}
          </div>
        </Card>
        <Card padded className="p-4">
          <span className="text-sm text-slate-400 font-bold block uppercase tracking-wider font-display">
        ...
        <Card padded className="p-4">
          <span className="text-sm text-slate-400 font-bold block uppercase tracking-wider font-display">Công nợ học phí cơ sở</span>
          <div className="text-2xl font-bold text-amber-600 font-display mt-1">
            {totalOutstanding.toLocaleString("vi-VN")} <span className="text-sm">đ</span>
          </div>
        </Card>
        <Card padded className="p-4">
          <span className="text-sm text-slate-400 font-bold block uppercase tracking-wider font-display">Ý thức cảnh báo (Warning)</span>
      ...
      <Card>
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => navigate("/facility/feedback")} className="text-sm text-brand-orange hover:text-brand-red font-bold">
            Xem tất cả phản hồi
          </button>
        </div>
            ...
            >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-slate-800">{tkt.campusName}</span>
