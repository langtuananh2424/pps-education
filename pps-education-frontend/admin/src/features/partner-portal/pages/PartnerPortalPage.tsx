    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-slate-500">Đại diện đơn vị:</span>
          <Select value={partnerFilter} onChange={(e) => setPartnerFilter(e.target.value)} className="bg-white border text-sm font-bold text-slate-700 px-3 py-1.5 rounded-lg focus:outline-none">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card>
          <span className="text-sm text-slate-400 font-bold block uppercase tracking-wider">Tổng số học sinh</span>
        ...
        <Card>
          <span className="text-sm text-slate-400 font-bold block uppercase tracking-wider">Số lớp triển khai</span>
          <div className="flex items-baseline gap-2 mt-2">
        ...
        <Card>
          <span className="text-sm text-slate-400 font-bold block uppercase tracking-wider">Điểm TB học phần (GPA)</span>
          <div className="flex items-baseline gap-2 mt-2">
        ...
        <Card>
          <span className="text-sm text-slate-400 font-bold block uppercase tracking-wider">Tỷ lệ Chuyên cần</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-2xl font-bold text-emerald-600 font-display">{activeStats.attendance}</span>
            <span className="text-sm text-emerald-500 font-semibold">Tốt</span>
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card padded={false} className="lg:col-span-2 overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
            <span className="text-sm font-bold text-slate-700 font-display">Kế hoạch & Đề cương giảng dạy</span>
            <span className="text-sm bg-slate-200 text-slate-600 px-2 py-0.5 rounded font-bold font-mono">Syllabus Approved</span>
          </div>
        ...
        <Card className="space-y-4">
          <h3 className="text-sm font-bold text-slate-400 block uppercase tracking-wider font-display border-b border-slate-100 pb-2">Báo cáo hợp tác giáo dục</h3>
          ...
          <div className="bg-slate-50 p-3.5 rounded-lg border border-slate-100 text-sm font-medium space-y-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-sm text-slate-400 font-normal">Tổng hợp bởi Phòng Đào tạo PPS Việt Nam</span>
              </div>
            </div>

            <button onClick={handleDownloadReport} className="w-full bg-brand-gradient hover:opacity-95 text-white font-semibold text-sm py-2 rounded-lg flex items-center justify-center gap-1.5 shadow-glow">
