...
/** Nhãn loại nhân sự dịch qua i18next namespace "hrm-employees" — xem src/i18n/locales/{vi,en}/hrm-employees.json. */
export function employeeTypeLabel(t: (key: string) => string, employeeType: string): string {
  return t(`employeeType.${employeeType}`);
}

/** Nhãn trạng thái nhân sự dịch qua i18next namespace "hrm-employees" — xem src/i18n/locales/{vi,en}/hrm-employees.json. */
export function employeeStatusLabel(t: (key: string) => string, status: string): string {
  return t(`employeeStatus.${status}`);
}
    ...
    <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-soft overflow-hidden flex flex-col h-full">
      <div className="px-5 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50 shrink-0">
        <div className="space-y-0.5">
          <span className="text-xs font-bold text-slate-700 font-display block">{t("employeeList.title")}</span>
          <p className="text-[10px] text-slate-400">{t("employeeList.subtitle")}</p>
        </div>
        <Button variant="primary" size="sm" onClick={onCreate} className="whitespace-nowrap shrink-0">
          <Plus className="w-3.5 h-3.5" />
          {t("employeeList.addButton")}
        </Button>
      </div>
      ...
      <div className="divide-y divide-slate-100 overflow-y-auto max-h-[620px] lg:max-h-[680px]">
        {loading ? (
          <div className="p-8 text-center text-slate-400 text-xs">{t("employeeList.loading")}</div>
        ) : employees.length === 0 ? (
          <EmptyState icon={Users} title={t("employeeList.emptyTitle")} description={t("employeeList.emptyDescription")} />
        ) : (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <Badge variant={employeeStatusVariants[emp.status]}>{employeeStatusLabel(t, emp.status)}</Badge>
                    </div>
                    <div className="flex items-center gap-1.5 text-[10px] text-slate-400 mt-1">
                      <span className="font-mono">{emp.employeeCode}</span>
                      <span>•</span>
                      <span className="text-brand-orange font-bold">{employeeTypeLabel(t, emp.employeeType)}</span>
                      {emp.positionName && (
                        <>
                          <span>•</span>
      ...
      {!loading && employees.length > 0 && (
        <Pagination
          ...
          itemLabel={t("employeeList.itemLabel")}
