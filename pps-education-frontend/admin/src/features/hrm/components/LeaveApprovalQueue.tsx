  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-soft overflow-hidden">
      <div className="px-5 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between gap-3">
        <div>
          <span className="text-sm font-bold text-slate-700 font-display block">Hàng chờ xét duyệt</span>
          <p className="text-sm text-slate-400">Các đơn xin nghỉ chờ xét duyệt thuộc thẩm quyền của bạn.</p>
        </div>
        <span className="px-2 py-1 rounded-full text-sm font-bold bg-brand-orange text-white shrink-0">{leaveRequests.length}</span>
      </div>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center space-y-3">
            ...
            <div className="space-y-1">
              <h4 className="text-sm font-bold text-slate-700">Không có đơn nào chờ duyệt</h4>
              <p className="text-sm text-slate-400 max-w-xs mx-auto">Hiện tại không có yêu cầu nghỉ phép nào thuộc thẩm quyền của bạn.</p>
            </div>
          </div>
        ) : (
          leaveRequests.map((req) => (
            <div key={req.id} className="p-4 space-y-3">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h4 className="text-sm font-bold text-slate-900">{req.employeeFullName}</h4>
                    <span className="bg-slate-100 text-slate-600 text-sm font-bold px-2 py-0.5 rounded">{getLeaveTypeLabel(req.leaveType)}</span>
                    {req.departmentName && <span className="text-sm text-slate-400">{req.departmentName}</span>}
                  </div>
              ...
              <div className="space-y-2 pt-1 border-t border-slate-100">
                <div className="space-y-1">
                  <label className="text-sm uppercase font-bold tracking-wider text-slate-400 block font-mono">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded px-2 py-1 text-sm focus:outline-none"
                  />
                </div>
                ...
                <div className="flex gap-2 justify-end">
                  <button
                    ...
                    className={cn(
                      "px-2.5 py-1 hover:bg-rose-50 text-rose-600 border border-rose-200 text-sm font-bold rounded",
                  ...
                  <button
                    ...
                    className={cn(
                      "px-3 py-1 bg-brand-gradient hover:opacity-95 text-white text-sm font-bold rounded flex items-center gap-1",
                      decidingId === req.id && "opacity-50"
                    )}
                  >
