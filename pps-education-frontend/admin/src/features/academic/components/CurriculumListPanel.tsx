    ...
    <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-soft overflow-hidden flex flex-col h-full">
      <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between gap-3 bg-slate-50 shrink-0">
        <div className="space-y-0.5">
          <span className="text-sm font-bold text-slate-700 font-display block">Khung chương trình</span>
          <p className="text-sm text-slate-400">Chuẩn + bản tùy biến theo điểm trường</p>
        </div>
              ...
              >
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-sm font-mono font-bold uppercase text-brand-red bg-orange-50 border border-orange-100 px-1.5 py-0.5 rounded">{c.code}</span>
                  ...
                  <p className="text-sm text-slate-400 mt-1">{c.level ?? "—"} · {c.classCategory ?? "—"}</p>
                </div>
              </button>
            );
          })
