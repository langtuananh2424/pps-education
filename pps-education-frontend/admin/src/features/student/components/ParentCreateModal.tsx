...
export default function ParentCreateModal({ onClose, onCreated }: ParentCreateModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-xl max-h-[90vh] overflow-y-auto bg-white rounded-2xl border border-slate-200 shadow-xl">
        <div className="sticky top-0 bg-white px-5 py-4 border-b border-slate-100">
          ...
          <p className="text-sm text-slate-500 mt-1">Tạo hồ sơ phụ huynh trước, liên kết con em sau tại màn chi tiết.</p>
        </div>
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          {error && <div className="text-sm text-rose-600 bg-rose-50 border border-rose-100 p-2.5 rounded-lg">{error}</div>}

          <div className="space-y-2 border-t border-slate-100 pt-4">
            <span className="text-xs font-bold uppercase text-slate-500 tracking-wide">Ảnh đại diện</span>
            <AvatarUploadField
          ...
          <div className="space-y-2">
            <span className="text-xs font-bold uppercase text-slate-500 tracking-wide">Tài khoản</span>
            <AccountSelector value={account} onChange={setAccount} submitAttempted={submitAttempted} />
          </div>

          <div className="space-y-3 border-t border-slate-100 pt-4">
            <span className="text-xs font-bold uppercase text-slate-500 tracking-wide">Thông tin phụ huynh</span>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-semibold rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-50">
              Hủy
            </button>
            <button
              type="submit"
              disabled={submitting}
              className="px-4 py-2 text-sm font-bold rounded-lg bg-brand-gradient text-white shadow-glow hover:opacity-95 disabled:opacity-50 flex items-center gap-1.5"
            >
