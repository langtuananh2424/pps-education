...
export default function ParentCreateModal({ onClose, onCreated }: ParentCreateModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!account.userId && (!account.newAccount?.username || !account.newAccount?.email || !account.newAccount?.fullName)) {
      setError(t("parentCreateModal.validationError"));
      return;
    }
    setSubmitting(true);
    setError(null);
    try {
      ...
      setError(err instanceof ApiError ? err.message : t("parentCreateModal.createError"));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-white rounded-2xl border border-slate-200 shadow-xl">
        <div className="sticky top-0 bg-white px-5 py-4 border-b border-slate-100">
          <h3 className="text-sm font-bold font-display text-slate-900">{t("parentCreateModal.title")}</h3>
          <p className="text-[11px] text-slate-500 mt-1">{t("parentCreateModal.description")}</p>
        </div>
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          {error && <div className="text-xs text-rose-600 bg-rose-50 border border-rose-100 p-2.5 rounded-lg">{error}</div>}

          <div className="space-y-2 border-t border-slate-100 pt-4">
            <span className="text-[10px] font-bold uppercase text-slate-500">{t("parentCreateModal.avatarLabel")}</span>
            <AvatarUploadField
          ...
          <div className="space-y-2">
            <span className="text-[10px] font-bold uppercase text-slate-500">{t("parentCreateModal.accountSectionTitle")}</span>
            <AccountSelector value={account} onChange={setAccount} submitAttempted={submitAttempted} />
          </div>

          <div className="space-y-3 border-t border-slate-100 pt-4">
            <span className="text-[10px] font-bold uppercase text-slate-500">{t("parentCreateModal.infoSectionTitle")}</span>
            <div className="grid grid-cols-2 gap-3">
              <input value={form.occupation} onChange={(e) => setForm({ ...form, occupation: e.target.value })} placeholder={t("parentCreateModal.occupationPlaceholder")} className={inputClass} />
              <input value={form.workplace} onChange={(e) => setForm({ ...form, workplace: e.target.value })} placeholder={t("parentCreateModal.workplacePlaceholder")} className={inputClass} />
              <input value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} placeholder={t("parentCreateModal.addressPlaceholder")} className={`${inputClass} col-span-2`} />
              <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder={t("parentCreateModal.notesPlaceholder")} rows={2} className={`${inputClass} col-span-2`} />
            </div>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            >
              ...
              {submitting ? t("parentCreateModal.creating") : t("parentCreateModal.createButton")}
            </button>
          </div>
        </form>
      </div>
