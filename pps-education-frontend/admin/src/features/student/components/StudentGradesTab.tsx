    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 grid grid-cols-2 gap-4">
        <div className="space-y-1">
          <label className="text-sm font-bold uppercase text-slate-400 tracking-wider">Điểm Giữa Kỳ</label>
          <input
            type="number"
            step="0.1"
            min="0"
            ...
            className="w-full bg-white border border-slate-200 rounded px-2.5 py-1.5 text-sm focus:outline-none font-bold"
          />
        </div>
        <div className="space-y-1">
          <label className="text-sm font-bold uppercase text-slate-400 tracking-wider">Điểm Cuối Kỳ</label>
          <input
            type="number"
            step="0.1"
            min="0"
      ...
      <div className="space-y-1">
        <label className="text-sm font-bold uppercase text-slate-400 tracking-wider block">Lời khuyên của giáo vụ / Ghi chú</label>
        <textarea
          placeholder="Nhập ghi chú học bạ, nhận xét tổng kết..."
          value={comments}
          onChange={(e) => setComments(e.target.value)}
          rows={3}
          className="w-full bg-white border border-slate-200 rounded p-2 text-sm focus:outline-none"
        />
      </div>

      <button type="submit" className="w-full bg-brand-orange hover:bg-brand-orange/90 text-white text-sm font-bold py-2 rounded-lg flex items-center justify-center gap-1.5 transition-all">
        <Save className="w-4 h-4 text-white" />
        <span>Cập nhật Học bạ & Điểm số</span>
      </button>

      {grade ? (
        <div className="bg-slate-50 border border-slate-200/60 p-3.5 rounded-xl space-y-2 mt-4 text-sm">
          <span className="text-sm font-bold text-slate-400 uppercase tracking-wider block">Báo cáo học tập chính thức hiện tại</span>
          <div className="grid grid-cols-3 gap-2 text-center text-sm">
            <div className="bg-white p-1.5 rounded border border-slate-100">
              <span className="text-sm text-slate-400 block">Giữa kỳ</span>
              <span className="font-bold text-slate-800 font-mono">{grade.midtermScore ?? "-"}</span>
            </div>
            <div className="bg-white p-1.5 rounded border border-slate-100">
              <span className="text-sm text-slate-400 block">Cuối kỳ</span>
            ...
            <div className="bg-white p-1.5 rounded border border-emerald-100 bg-emerald-50/20">
              <span className="text-sm text-emerald-800 block">Trung bình</span>
              <span className="font-bold text-emerald-600 font-mono">{grade.averageScore ?? "-"}</span>
            </div>
          </div>
          {grade.teacherComments && <p className="text-sm text-slate-500 italic mt-2 border-t pt-2">" {grade.teacherComments} "</p>}
          <p className="text-[9px] text-slate-400 text-right mt-1">Lần cuối: {grade.updatedAt}</p>
        </div>
      ) : (
        <div className="p-4 border border-dashed rounded-xl text-center text-sm text-slate-400 italic">
