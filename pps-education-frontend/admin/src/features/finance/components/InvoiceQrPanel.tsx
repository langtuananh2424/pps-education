  ...
  return (
    <div className="space-y-4 text-center">
      <div className="border-b pb-2 text-left">
        <span className="text-sm font-mono font-bold text-slate-400">CHI TIẾT THANH TOÁN</span>
      ...
      <div className="flex flex-col items-center p-4 bg-slate-50 rounded-lg border border-slate-100">
        <span className="text-sm font-bold text-slate-400 block uppercase tracking-widest font-mono mb-2">Mã QR ngân hàng động (VietQR API)</span>
        ...
        <div className="text-sm font-semibold text-slate-700 mt-3 font-mono">Học phí: {invoice.finalAmount.toLocaleString("vi-VN")} đ</div>
      ...
      {invoice.status !== "PAID" ? (
        <button
          ...
          className="w-full bg-brand-gradient hover:opacity-95 text-white font-bold text-sm py-2 rounded-lg shadow-glow flex items-center justify-center gap-1"
        >
          <Check className="w-4 h-4" />
          Giả lập webhook Ngân hàng (Xác nhận)
        </button>
      ) : (
        <div className="p-3 bg-emerald-50 border border-emerald-100 text-emerald-800 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5">
