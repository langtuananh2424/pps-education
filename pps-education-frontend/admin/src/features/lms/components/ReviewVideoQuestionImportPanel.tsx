...
const REFLEX_HEADERS = ["Mốc thời gian (giây)", "Nội dung câu hỏi", "Thời lượng ghi âm tối đa (giây)", "Số lượt nộp tối đa", "Thứ tự hiển thị"];
const REFLEX_SAMPLE_ROWS: string[][] = [
  ["30", "Describe your favorite hobby.", "60", "3", "1"],
  ["120", "What did you do last weekend?", "45", "", "2"]
];

const CONNECTION_HEADERS = ["Nội dung câu hỏi", "Đáp án A", "Đáp án B", "Đáp án C", "Đáp án D", "Đáp án E", "Đáp án đúng", "Thứ tự hiển thị"];
...
export default function ReviewVideoQuestionImportPanel({ videoId, videoType, onImported }: ReviewVideoQuestionImportPanelProps) {
  ...
  const headers = videoType === "REFLEX" ? REFLEX_HEADERS : CONNECTION_HEADERS;
  ...
  const handleDownloadTemplate = () => {
    ...
    downloadBlob(blob, videoType === "REFLEX" ? "mau-cau-hoi-video-phan-xa.xlsx" : "mau-cau-hoi-video-ket-noi.xlsx");
  };
  ...
  return (
    <div className="space-y-3">
      ...
      >
        ...
        <span className="text-xs font-bold text-slate-700">{submitting ? "Đang nhập..." : "Click để chọn file (.xlsx)"}</span>
      </button>
