    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      <div className="bg-gradient-to-r from-orange-50 to-amber-50/50 border border-orange-200/60 p-4 rounded-xl flex items-start gap-3 shadow-sm">
        ...
        <div className="space-y-1">
          <span className="text-sm font-bold text-brand-red uppercase tracking-wider block">TÙY CHỈNH TÀI KHOẢN (USER OVERRIDES - UC-04)</span>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm space-y-3">
        <label className="text-sm uppercase font-bold text-slate-500 block">Bước 1 — Tìm và chọn tài khoản</label>
      ...
      {userDetail && effective && !loading && (
        <>
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm space-y-2">
            <span className="text-sm uppercase font-bold text-slate-500 block">
            ...
            <div className="flex flex-wrap gap-1.5">
              {effective.permissions.length === 0 ? (
                ...
                effective.permissions.map((code) => (
                  <code key={code} className="text-sm font-mono font-bold text-brand-red bg-orange-50/60 border border-orange-100 px-1.5 py-0.5 rounded">
                    {code}
                  </code>
                ))
              )}
