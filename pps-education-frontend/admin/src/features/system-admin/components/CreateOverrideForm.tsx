...
export default function CreateOverrideForm({ permissions, onSubmit }: CreateOverrideFormProps) {
  ...
  return (
    <div className="lg:col-span-1 bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-4 h-fit">
      <span className="text-sm font-bold text-slate-800 uppercase tracking-wider block border-b pb-2">Bước 2 — Thiết lập ngoại lệ</span>

      <form onSubmit={handleSubmit} className="space-y-4">
        {error && <div className="text-sm text-rose-600 bg-rose-50 border border-rose-100 p-2 rounded-lg">{error}</div>}

        <div className="space-y-1">
          <label className="text-sm uppercase font-bold text-slate-500 block">Chọn Mã Quyền hạt nhân</label>
          <Select value={permissionId} onChange={(e) => setPermissionId(e.target.value)} required className="w-full bg-slate-50 border border-slate-200 text-sm p-2.5 rounded-lg focus:outline-none">
        ...
        <div className="space-y-1">
          <label className="text-sm uppercase font-bold text-slate-500 block">Loại điều chỉnh</label>
        ...
        <div className="space-y-1">
          <label className="text-sm uppercase font-bold text-slate-500 block">Thời hạn áp dụng (tùy chọn)</label>
          <DatePicker value={expiresAt} onChange={setExpiresAt} min={TODAY_ISO} />
        </div>

        <div className="space-y-1">
          <label className="text-sm uppercase font-bold text-slate-500 block">Mục đích điều chỉnh / Lý do ủy quyền</label>
          <textarea
            value={reason}
            ...
            required
            rows={3}
            className="w-full bg-slate-50 border border-slate-200 text-sm p-2.5 rounded-lg focus:outline-none text-slate-700"
          />
        </div>
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full bg-brand-gradient hover:opacity-90 text-white font-bold text-sm py-2 px-4 rounded-lg flex items-center justify-center gap-1 transition-all shadow-md disabled:opacity-50"
        >
