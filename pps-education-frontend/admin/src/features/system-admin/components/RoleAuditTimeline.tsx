...
export default function RoleAuditTimeline({ roleName, logs }: RoleAuditTimelineProps) {
  return (
    <div className="space-y-4">
      <div>
        <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wider block">Nhật ký thay đổi vai trò (Audit Log)</h4>
        <p className="text-sm text-slate-400 mt-0.5">Lưu vết truy cập và thiết lập bảo mật riêng cho vai trò "{roleName}".</p>
      </div>

      {logs.length === 0 ? (
        <div className="py-12 border rounded-xl bg-slate-50 text-center text-slate-400 text-sm italic">
