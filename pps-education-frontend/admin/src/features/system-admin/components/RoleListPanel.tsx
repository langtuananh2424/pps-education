    ...
    <div className="w-full md:w-80 lg:w-96 border-r border-slate-200 flex flex-col bg-slate-50/50">
      <div className="p-4 border-b border-slate-200 space-y-3 bg-white">
        <div className="flex items-center justify-between">
          <span className="text-sm font-bold text-slate-800 uppercase tracking-wide">Danh sách Vai trò ({roles.length})</span>
          <button
            onClick={onAddNew}
            className="bg-brand-gradient hover:opacity-90 text-white p-1.5 rounded-lg flex items-center gap-1 text-sm font-semibold transition-all shadow-sm"
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-slate-50 border border-slate-200 text-sm pl-9 pr-3 py-2 rounded-lg focus:outline-none focus:ring-1 focus:ring-brand-orange text-slate-800 font-sans"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-slate-150 p-2 space-y-1 bg-slate-50/30 max-h-[580px]">
        {loading ? (
          ...
          <div className="py-12 text-center text-slate-400 text-sm italic">Không tìm thấy vai trò phù hợp</div>
        ) : (
          filtered.map((role) => {
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <span className={cn("text-sm font-bold leading-tight", isSelected ? "text-brand-red" : "text-slate-900")}>
                    {role.name}
                  </span>
                  <Badge variant={role.isSystem ? "info" : "neutral"}>{role.isSystem ? "Hệ thống" : "Tùy chỉnh"}</Badge>
                </div>

                {role.description && <p className="text-sm text-slate-400 line-clamp-2 leading-normal">{role.description}</p>}

                <div className="flex items-center gap-3 pt-1 text-sm text-slate-500 font-medium">
