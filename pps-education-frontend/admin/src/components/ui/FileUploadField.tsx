...
export default function FileUploadField({ value, onChange, onUpload, onFileSize, accept, placeholder, disabled }: FileUploadFieldProps) {
  ...
  // Bổ sung ngoài SDD gốc, đã xác nhận với người dùng 2026-08-23 — trước đây file đã chọn chỉ hiện URL
  // dạng chữ (dù là ảnh) — GV không xem trước được đã chọn đúng ảnh chưa. Với ảnh (accept chứa "image"),
  // hiện thêm 1 thumbnail nhỏ cạnh URL — áp dụng chung cho MỌI chỗ dùng field này để chọn ảnh (không chỉ
  // riêng "Nghe chọn hình"), không cần sửa từng nơi gọi.
  const isImage = accept?.includes("image");
  // Cùng lý do như ảnh ở trên — audio (accept chứa "audio") cũng chỉ hiện URL dạng chữ, GV không nghe
  // thử được để kiểm tra đã upload đúng file chưa, phải mở tab mới/tải về mới nghe được. Thêm player
  // <audio controls> ngay tại chỗ, áp dụng chung mọi nơi dùng field này để chọn audio.
